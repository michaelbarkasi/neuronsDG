...
test_est <- autocor.ests.kilo4
test_est$n_sims_per_neurons <- 1
test_est$estimates <- test

autocorr_results2 <- analyze.autocorr(
  test_est,
  covariate = c("hemi","genotype"),
  n_bs = 1e4
)
...
df2 <- data.frame(
  value = unlist(autocorr_results2, use.names = FALSE),
  group = rep(names(autocorr_results2), each = nrow(autocorr_results2))
)
...
axis_size <- 12 
legend_size <- 10
ggplot2::ggplot(df2, ggplot2::aes(x = value, fill = group)) +
  ggplot2::geom_density(alpha = 0.6) +
  #scale_y_continuous(transform = "log1p") +
  ggplot2::labs(title = "Expected Time Constant", x = "ms", y = "Density") +
  ggplot2::theme_minimal() + 
...
print(colMeans(autocorr_results))
print(colMeans(autocorr_results2))
```